...
# Эталонное решение преподавателя: нормализация, стандартизация и сборка аналитической витрины в R.
...
# 13. Контрольная сверка преподавателя -------------------------------------------
expected <- fromJSON("docs/r_transformation_expected_checks.json")

actual <- list(
  raw_sales_rows = as.integer(nrow(sales)),
  sales_ready_for_report_rows = as.integer(nrow(sales_datamart)),
  sales_excluded_rows = as.integer(sum(sales_work$exclude_from_report, na.rm = TRUE)),
  products_rows_raw = as.integer(nrow(products)),
  products_unique_after_dedup = as.integer(nrow(products_dedup)),
  clients_rows_raw = as.integer(nrow(clients)),
  clients_unique_after_dedup = as.integer(nrow(clients_dedup)),
  regions_unique_after_dedup = as.integer(nrow(regions_dedup)),
  report_by_region_channel_rows = as.integer(nrow(report_by_region_channel)),
  total_revenue = as.numeric(round(sum(sales_datamart$revenue, na.rm = TRUE), 2)),
  total_margin = as.numeric(round(sum(sales_datamart$margin, na.rm = TRUE), 2)),
  channels = sort(unique(na.omit(sales_datamart$channel))),
  categories = sort(unique(na.omit(sales_datamart$category))),
  min_revenue_minmax = as.numeric(round(min(sales_datamart$revenue_minmax, na.rm = TRUE), 6)),
  max_revenue_minmax = as.numeric(round(max(sales_datamart$revenue_minmax, na.rm = TRUE), 6)),
  mean_revenue_zscore_rounded = as.numeric(round(mean(sales_datamart$revenue_zscore, na.rm = TRUE), 6))
)

check_scalar <- function(name) {
  tibble(
    check = name,
    expected = as.character(expected[[name]]),
    actual = as.character(actual[[name]]),
    ok = identical(expected[[name]], actual[[name]]) || as.character(expected[[name]]) == as.character(actual[[name]])
  )
}

scalar_checks <- c(
  "raw_sales_rows",
  "sales_ready_for_report_rows",
  "sales_excluded_rows",
  "products_rows_raw",
  "products_unique_after_dedup",
  "clients_rows_raw",
  "clients_unique_after_dedup",
  "regions_unique_after_dedup",
  "report_by_region_channel_rows",
  "total_revenue",
  "total_margin",
  "min_revenue_minmax",
  "max_revenue_minmax",
  "mean_revenue_zscore_rounded"
)

control_check <- map_dfr(scalar_checks, check_scalar) |>
  bind_rows(
    tibble(
      check = "channels",
      expected = paste(expected$channels, collapse = ", "),
      actual = paste(actual$channels, collapse = ", "),
      ok = identical(expected$channels, actual$channels)
    ),
    tibble(
      check = "categories",
      expected = paste(expected$categories, collapse = ", "),
      actual = paste(actual$categories, collapse = ", "),
      ok = identical(expected$categories, actual$categories)
    )
  )

write_csv(control_check, "outputs/r_datamart_control_check.csv")
print(control_check)

if (!all(control_check$ok)) {
  stop("Часть контрольных значений не совпала. Проверьте трансформации выше.")
}

cat("Все контрольные значения R-витрины совпали.\n")
